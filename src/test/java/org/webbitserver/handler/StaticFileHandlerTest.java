...
public class StaticFileHandlerTest {
  ...
  public void shouldSupportUnboundedEndRangeRequests() throws Exception {
    ...
    assertEquals("bytes 0-" + (contents.length() - 1) + "/" + contents.length(), response
  ...
  public void shouldSupportUnboundedStartRangeRequests() throws Exception {
    ...
    assertEquals("bytes 31-" + (contents.length() - 1) + "/" + contents.length(), response
        .header("Content-Range"));
  ...
  public void shouldGuessMimeTypeFromExtension() throws Exception {
    ...
    assertEquals("text/html; charset=UTF-8", handle(request("/a/b/")).header("Content-Type")); // <--
                                                                                               // welcome
                                                                                               // file
    assertEquals("text/javascript; charset=UTF-8", handle(request("/a/b/foo.js")).header(
        "Content-Type"));
    assertEquals("text/javascript; charset=UTF-8", handle(request("/a/b/foo.css.js")).header(
        "Content-Type"));
  ...
  /**
   * End to end integration test, that fires up a real web server and uses HTTP to check the
   * responses.
   */
  @Test
  public void shouldWorkInRealServer() throws IOException, InterruptedException, ExecutionException {
    ...
    WebServer webServer = createWebServer(59504).add(handler).start().get();
    try {
  ...
  public void createWorkingDir() throws IOException {
    dir =
        new File(System.getProperty("java.io.tmpdir"), getClass().getName() + "-" + Math.random());
  ...
  private void assertReturnedWithStatusAndContainsContent(int expectedStatus, String content,
  ...
  private void assertReturnedLocationHeaderEqualTo(String locationHeaderValue,
      StubHttpResponse response) {
