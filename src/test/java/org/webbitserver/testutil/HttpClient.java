...
public class HttpClient {
  ...
  public static URLConnection httpGetAcceptCompressed(WebServer webServer, String path)
      throws IOException {
  ...
  public static URLConnection httpPost(WebServer webServer, String path, String body)
      throws IOException {
    URL url = new URL(webServer.getUri().toURL(), path);
  ...
  public static URLConnection httpPostCompressed(WebServer webServer, String path, String body)
      throws IOException {
    URL url = new URL(webServer.getUri().toURL(), path);
    HttpURLConnection urlConnection = (HttpURLConnection) url.openConnection();
  ...
  public static String contents(URLConnection urlConnection) throws IOException {
    ...
    if (length != -1 && length != read) {
      throw new IOException("Content-Length header (" + length + ") did not match actual length ("
          + read + ")");
    }
