...
/**
 * This example how to verify username/passwords in the background without blocking the main Webbit
 * thread.
 */
public class AsyncPasswordsExample {
  ...
  public static void main(String[] args) throws ExecutionException, InterruptedException {
    WebServer webServer =
        createWebServer(45454).add(new BasicAuthenticationHandler(new SlowPasswordAuthenticator()))
            .add("/whoami", new WhoAmIHttpHandler())
            .add("/whoami-ws", new WhoAmIWebSocketHandler()).add(
                new StaticFileHandler("src/test/java/samples/authentication/content")).start()
            .get();
  ...
  private static class SlowPasswordAuthenticator implements PasswordAuthenticator {
    @Override
    public void authenticate(HttpRequest request, final String username, final String password,
        final ResultCallback callback, final Executor handlerExecutor) {
      // Submit some slow work to a background thread, so we don't block the main Webbit thread.
      backgroundAuthenticatorThread.execute(new BackgroundWorker(username, password, callback,
          handlerExecutor));
    }
  }
  ...
  static class BackgroundWorker implements Runnable {
    ...
    public BackgroundWorker(String username, String password,
        PasswordAuthenticator.ResultCallback callback, Executor handlerExecutor) {
