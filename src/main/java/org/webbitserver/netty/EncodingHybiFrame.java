...
import static org.webbitserver.netty.HybiWebSocketFrameDecoder.*;
...
public class EncodingHybiFrame {
  ...
  public EncodingHybiFrame(int opcode, boolean fin, int rsv, byte[] maskingKey,
      ChannelBuffer fragment) {
    this.opcode = opcode;
    this.fin = fin;
    this.rsv = rsv;
  ...
  public ChannelBuffer encode() throws TooLongFrameException {
    ...
    if (opcode == Opcodes.OPCODE_PING && length > 125) {
      throw new TooLongFrameException(
          "invalid payload for PING (payload length must be <= 125, was " + length);
    }
