...
public class WebSocketClient implements WebSocket {
  ...
  public Future<WebSocketClient> start() {
    ...
    FutureTask<WebSocketClient> future =
        new FutureTask<WebSocketClient>(new Callable<WebSocketClient>() {
          @Override
          public WebSocketClient call() throws Exception {
            final byte[] outboundMaskingKey =
                new byte[] {randomByte(), randomByte(), randomByte(), randomByte()};

            bootstrap =
                new ClientBootstrap(new NioClientSocketChannelFactory(Executors
            ...
            bootstrap.setPipelineFactory(new ChannelPipelineFactory() {
              public ChannelPipeline getPipeline() throws Exception {
                ...
                pipeline.addLast("handshakeHandler",
                    new HandshakeChannelHandler(outboundMaskingKey));
                return pipeline;
              }
            });
  ...
  private HttpRequest createNettyHttpRequest() {
    HttpRequest request =
        new DefaultHttpRequest(HttpVersion.HTTP_1_1, HttpMethod.GET, getPath(uri));
    request.setHeader(HttpHeaders.Names.HOST, remoteAddress.getHostName() + ':'
        ...
        new FutureTask<WebSocketClient>(new Callable<WebSocketClient>() {
          ...
          public WebSocketClient call() throws Exception {
            try {
              ...
              exceptionHandler.uncaughtException(Thread.currentThread(), WebbitException
                  .fromException(e, channel));
            }
            return WebSocketClient.this;
          }
        });
  ...
  public WebSocketClient reconnectEvery(long reconnectIntervalMillis) {
    webSocketHandler =
        new ReconnectingWebSocketHandler(webSocketHandler, WebSocketClient.this,
  ...
  private class HandshakeChannelHandler extends SimpleChannelUpstreamHandler {
    ...
    public void messageReceived(ChannelHandlerContext ctx, MessageEvent e) throws Exception {
      ...
      adjustPipelineToWebSocket(ctx, e, HybiWebSocketFrameDecoder.clientSide(outboundMaskingKey),
          new HybiWebSocketFrameEncoder());
    }
    ...
    private void adjustPipelineToWebSocket(ChannelHandlerContext ctx, MessageEvent messageEvent,
        ChannelHandler webSocketFrameDecoder, ChannelHandler webSocketFrameEncoder) {
      NettyHttpRequest httpRequest =
          new NettyHttpRequest(messageEvent, request, nextId(), timestamp());
      final NettyWebSocketConnection webSocketConnection =
          new NettyWebSocketConnection(executor, httpRequest, ctx, outboundMaskingKey);
      webSocketConnection.setHybiWebSocketVersion(VERSION);

      ChannelHandler webSocketChannelHandler =
          new WebSocketConnectionHandler(executor, exceptionHandler, ioExceptionHandler,
