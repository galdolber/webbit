...
public class Hixie76 implements WebSocketVersion {
  ...
  public void prepareHandshakeResponse(NettyWebSocketConnection webSocketConnection) {
    ...
    res.addHeader(CONNECTION, org.jboss.netty.handler.codec.http.HttpHeaders.Names.UPGRADE);
    ...
    int a =
        (int) (Long.parseLong(key1.replaceAll("[^0-9]", "")) / key1.replaceAll("[^ ]", "").length());
    int b =
        (int) (Long.parseLong(key2.replaceAll("[^0-9]", "")) / key2.replaceAll("[^ ]", "").length());
