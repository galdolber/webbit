...
public class EmbeddedResourceHandler extends AbstractResourceHandler {
  // We're using File because it's good at dealing with path concatenation and slashes. Not actually
  // opening the file.
  ...
  @Override
  protected ResourceWorker createIOWorker(HttpRequest request, HttpResponse response,
      HttpControl control) {
  ...
  protected class ResourceWorker extends IOWorker {
    ...
    protected ResourceWorker(HttpRequest request, HttpResponse response, HttpControl control) {
      ...
      pathWithoutTrailingSlash =
          withoutQuery(isDirectory ? path.substring(0, path.length() - 1) : path);
    }
    ...
    protected byte[] directoryListingBytes() throws IOException {
      ...
      Iterable<FileEntry> files =
          ClassloaderResourceHelper.listFilesRelativeToClass(clazz, subdirectory);
