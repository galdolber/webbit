...
/**
 * Adds HTTP Basic authentication to a page. Users should provide an implementation of
 * UsernamePasswordAuthenticator to check the supplied credentials.
 * <p/>
...
public class BasicAuthenticationHandler implements HttpHandler {
  ...
  private void needAuthentication(HttpResponse response) {
    response.status(401).header("WWW-Authenticate", "Basic realm=\"" + realm + "\"").content(
        "Need authentication").end();
  }
