...
import static org.webbitserver.helpers.Hex.*;
...
public class SimpleLogSink implements LogSink {
  ...
  protected Appendable formatLogEntry(Appendable out, HttpRequest request, String action,
  ...
  protected Appendable formatHeader(Appendable out) throws IOException {
    out.append("#Log started at ").append(new Date().toString()).append(" (").append(
        String.valueOf(System.currentTimeMillis())).append(")").append(lineSeparator).append('#');
