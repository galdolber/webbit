...
public abstract class AbstractResourceHandler implements HttpHandler {
  static {
    // This is not an exhaustive list, just the most common types. Call registerMimeType() to add
    // more.
    ...
    mimeTypes.put("js", "text/javascript"); // Technically it should be application/javascript (RFC
                                            // 4329), but IE8 struggles with that
  ...
  @Override
  public void handleHttpRequest(final HttpRequest request, final HttpResponse response,
      final HttpControl control) throws Exception {
    // Switch from web thead to IO thread, so we don't block web server when we access the
    // filesystem.
    ioThread.execute(createIOWorker(request, response, control));
  }

  protected void serve(final String mimeType, final byte[] staticContents, HttpControl control,
      final HttpResponse response, final HttpRequest request, final String path) {
    ...
    control.execute(new Runnable() {
      ...
      public void run() {
        ...
        byte[] dynamicContents =
            templateEngine.process(staticContents, path, request
                .data(TemplateEngine.TEMPLATE_CONTEXT));
  ...
  private boolean maybeServeRange(HttpRequest request, ByteBuffer contents, HttpResponse response) {
    ...
    if (null != startString && null != endString) {
      ...
      if (start <= end) {
        serveRange(start, Math.min(contents.remaining() - 1, end), contents, response);
        return true;
      }
    } else if (null != startString) {
      serveRange(Integer.parseInt(startString), contents.remaining() - 1, contents, response);
      return true;
    } else if (null != endString) {
      int end = Integer.parseInt(endString);
      serveRange(contents.remaining() - end, contents.remaining() - 1, contents, response);
      return true;
    }
  ...
  protected void serveRange(int start, int end, ByteBuffer contents, HttpResponse response) {
    ...
    response.status(206).header("Content-Length", end - start + 1) // since its inclusive
        .header("Content-Range", "bytes " + start + "-" + end + "/" + contents.remaining());

    contents.limit(contents.position() + end + 1).position(contents.position() + start);
    response.content(contents).end();
  }

  protected abstract IOWorker createIOWorker(HttpRequest request, HttpResponse response,
      HttpControl control);
