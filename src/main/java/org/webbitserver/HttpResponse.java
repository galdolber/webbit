 ...
 * <p/>
 * IMPORTANT: The connection will remain open until {@link #end()} or {@link #error(Throwable)} is
 * called. Don't forget these!
 * 
 * @author Joe Walnes
 */
public interface HttpResponse {
  ...
  /**
   * Adds a Date (RFC 1123 format) HTTP header. Multiple HTTP headers can be added with the same
   * name.
   */
  ...
  /**
   * Writes immediately to the client without closing the connection. (The {@link #content(String)}
   * method caches content until {@link #end()} is called()
   * <p/>
  ...
  /**
   * Marks the response as ended. At this point any remaining data shall be flushed and the
   * connection closed.
   * <p/>
   * Every response should have either {@link #end()} or {@link #error(Throwable)} called. No more
   * operations should be performed on a response after these.
