...
public interface Endpoint<T> {
  /**
   * Start in background. This returns immediately, but the endpoint may still not be ready to
   * accept incoming requests (or have established a connection - if this is a client). To wait
   * until it's fully started, call {@link java.util.concurrent.Future#get()} on the returned
   * future.
   */
  Future<? extends T> start();

  /**
   * Stop in background. This returns immediately, but the endpoint may still be shutting down. To
   * wait until it's fully stopped, call {@link java.util.concurrent.Future#get()} on the returned
   * future.
   */
  ...
  /**
   * What to do when an exception occurs when attempting to read/write data from/to the underlying
   * connection. e.g. If an HTTP request disconnects before it was expected.
   * <p/>
   * Defaults to using {@link org.webbitserver.handler.exceptions.SilentExceptionHandler} as this is
   * a common thing to happen on a network, and most systems should not care.
   */
