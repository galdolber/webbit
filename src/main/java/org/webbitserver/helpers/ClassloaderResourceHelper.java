...
public class ClassloaderResourceHelper {
  ...
  /**
   * Lists all files and directories relative to the classpath entry (jar or folder) containing
   * clazz, directly under subdirectory.
   * <p/>
   * If clazz org.webbitserver.Foo is in folder /some/path/org/webbitserver/Foo.clazz, will list
   * files in /some/path/$subdirectory <br />
   * If clazz org.webbitserver.Foo is in jar /some/path/webbit.jar, will list files in
   * /some/path/webbit.jar:$subdirectory
   * 
   * @param clazz Any class within the classpath entry to be listed.
   * @param subdirectory Subdirectory to list. Must be an absolute path, relative to the top level
   *          of the classpath entry.
   * @return List of files directly in subdirectory, under the classpath entry containing clazz.
   * @throws IOException
   */
  public static Iterable<FileEntry> listFilesRelativeToClass(Class<?> clazz, String subdirectory)
      throws IOException {
