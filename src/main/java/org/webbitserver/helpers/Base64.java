 ...
 * <p/>
 * Despite there being a gazillion other Base64 implementations out there, this has been written as
 * part of XStream as it forms a core part but is too trivial to warrant an extra dependency.
 * <p/>
 * This meets the standard as described in RFC 1521, section 5.2
 * <http://www.freesoft.org/CIE/RFC/1521/7.htm>, allowing other Base64 tools to manipulate the data.
 * <p/>
 * This code originally came from the XStream http://xstream.codehaus.org project by Joe Walnes.
 * Relicensed to Webbit.
 */
public class Base64 {
  ...
  //
  // 3) Each of these 4 groups of 6 bits are converted back to a number, which will fall in the
  // range of 0 - 63.
  //
  // 4) Each of these 4 numbers are converted to an alphanumeric char in a specified mapping table,
  // to create
  ...
  public static String encode(byte[] input) {
    ...
    for (int i = 0; i < input.length; i += 3) {
      ...
      int oneBigNumber =
          (input[i] & 0xff) << 16 | (remaining <= 1 ? 0 : input[i + 1] & 0xff) << 8
  ...
  public static byte[] decode(String input) {
    try {
      ...
      for (int i = 0; i < input.length(); i += 4) {
        ...
        int oneBigNumber =
            (a[0] & 0x3f) << 18 | (a[1] & 0x3f) << 12 | (a[2] & 0x3f) << 6 | (a[3] & 0x3f);
        for (int j = 0; j < 3; j++) {
          if (a[j + 1] >= 0) {
