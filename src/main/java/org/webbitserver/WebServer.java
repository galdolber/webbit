 ...
 * <p/>
 * <p>
 * As with many of the interfaces in webbitserver, setter style methods return a reference to this,
 * to allow for simple initialization using method chaining.
 * </p>
 * <p/>
...
public interface WebServer extends Endpoint<WebServer> {
  ...
  /**
   * Add an HttpHandler. When a request comes in the first HttpHandler will be invoked. The
   * HttpHandler should either handle the request, or pass the request onto the next HttpHandler
   * (using {@link HttpControl#nextHandler()}). This is repeated until a HttpHandler returns a
   * response. If there are no remaining handlers, the webserver shall return 404 NOT FOUND to the
   * browser.
   * <p/>
   ...
   * {@code add(new PathMatchHandler(path, newHttpToWebSocketHandler(handler)))}.
   * 
   ...
   * <p/>
   * This is shortcut for
   * {@code add(new PathMatchHandler(path, newHttpToEventSourceHandler(handler)))}.
   * 
   * @see HttpHandler
   * @see #add(HttpHandler)
  ...
  /**
   * Number of milliseconds before a stale HTTP keep-alive connection is closed by the server. A
   * HTTP connection is considered stale if it remains open without sending more data within the
   * timeout window.
   */
